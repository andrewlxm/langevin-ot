...
#compute gradient of joint objective w.r.t. X and weights
def compute_T_OT_weighted(U, X, weights_X, epsilon):
    """Computes Entropic OT map with custom weights on target points X."""
    m = U.shape[0]
    n = X.shape[0]
    # Uniform weights for source
    a = jnp.ones((m,)) / m
    # Custom weights for target
    b = weights_X
    
    geom = pointcloud.PointCloud(U, X, epsilon=epsilon)
    prob_ot = linear_problem.LinearProblem(geom, a=a, b=b)
    
    # Use implicit_diff=None to avoid linear solver issues in joint opt
    sinkhorn_engine = linear.sinkhorn.Sinkhorn(threshold=1e-1, max_iterations=500, implicit_diff=None)
    out_ot = sinkhorn_engine(prob_ot)
    
    P_mat = out_ot.matrix
    # Normalize by row sums to get the map
    P_norm = P_mat / (jnp.sum(P_mat, axis=1, keepdims=True) + 1e-10)
    return jnp.dot(P_norm, X)
